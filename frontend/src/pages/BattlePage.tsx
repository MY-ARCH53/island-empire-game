...
function BattlePage() {
  ...
  const handleBuyShield = async () => {
    ...
    try {
      ...
      // Kalkan süresini ordu state'ine yansıt
      const armyRes = await battleAPI.getArmy(user.id);
      setArmy(armyRes.data.data.army);
    } catch (e: any) {
      showToast(e.response?.data?.message || 'Hata', 'error');
    ...
    <div style={{
            ...
            {pirates.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {pirates.map((p: any) => {
                  ...
                  return (
                    <div key={p.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ color: '#64748b', fontSize: 12 }}>⚔️ {p.power} güç &nbsp;|&nbsp; 💰 {p.reward_gold} ödül</p>
                      </div>
                      <button
                        disabled={!canAttack}
            ...
            {pvpTargets.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {pvpTargets.map((p: any) => {
                  ...
                  const canAttack = army && army.total_power > 0 && !shielded;
                  return (
      ...
      {battleTarget && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
          <div onClick={() => { if (!battleResult) closeBattle(); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)' }} />
          <div style={{ position: 'relative', zIndex: 1, background: '#0f172a', borderRadius: '20px 20px 0 0', border: '1px solid rgba(255,255,255,0.1)', padding: '20px 18px 32px' }}>
            ...
            {!battleResult ? (
              <>
                <h2 style={{ fontWeight: 800, fontSize: 18, marginBottom: 16, textAlign: 'center' }}>⚔️ Savaşa Hazır mısın?</h2>
                ...
                <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 12, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: '#f59e0b' }}>
                  {battleType === 'pirate'
                    ? `Ödül: 💰 ${battleTarget.reward_gold}${battleTarget.reward_wood > 0 ? ` · 🌲 ${battleTarget.reward_wood}` : ''}`
                  ...
                  {battleResult.winner === 'attacker' && battleResult.rewards && (
                    <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 12, padding: '10px 14px' }}>
                      ...
                      <p style={{ color: '#f1f5f9', fontSize: 15, fontWeight: 700 }}>
                        ...
                        {battleResult.rewards.wood > 0 && ` · 🌲 ${battleResult.rewards.wood}`}
                        {battleResult.rewards.food > 0 && ` · 🍎 ${battleResult.rewards.food}`}
                      </p>
                    </div>
                  )}
                </div>
