...
# Faz F11 — item satışı. data: {itemName, price, silver}.
signal online_sell_result(success: bool, data: Dictionary, message: String)
...
func sell_online_item(inventory_item_id: int) -> void:
	if GameManager.jwt_token == "":
		online_sell_result.emit(false, {}, "Oturum yok.")
		return
	var http := HTTPRequest.new()
	add_child(http)
	http.request_completed.connect(_on_sell_completed.bind(http))
	var body := JSON.stringify({"inventoryItemId": inventory_item_id})
	var headers := [
		"Content-Type: application/json",
		"Authorization: Bearer " + GameManager.jwt_token,
	]
	var err := http.request(_api_base() + "/online/sell-item", headers, HTTPClient.METHOD_POST, body)
	if err != OK:
		online_sell_result.emit(false, {}, "Bağlantı hatası")
		http.queue_free()

func _on_sell_completed(_result: int, code: int, _headers: PackedStringArray, body: PackedByteArray, http: HTTPRequest) -> void:
	http.queue_free()
	var json := JSON.new()
	if json.parse(body.get_string_from_utf8()) != OK:
		online_sell_result.emit(false, {}, "Sunucu yanıtı okunamadı")
		return
	var data = json.get_data()
	if code == 200 and typeof(data) == TYPE_DICTIONARY and data.get("success", false):
		online_sell_result.emit(true, data.get("data", {}), str(data.get("message", "")))
	else:
		var msg := "Satılamadı"
		if typeof(data) == TYPE_DICTIONARY:
			msg = str(data.get("message", msg))
		online_sell_result.emit(false, {}, msg)
